...
// Estes testes precisam comecar deslogados, entao descartam o storageState
// que o projeto `setup` grava.
...
// Este comeca autenticado, herdando o storageState do projeto `setup`.
test.describe('Authentication (com sessao)', () => {
  test('logout clears session and redirects to login', async ({ page }) => {
    await page.goto('/')
    await page.waitForLoadState('networkidle')

    // Confirma que a sessao herdada esta valendo
    await expect(
      page.locator('h1').filter({ hasText: /Dashboard/ }),
    ).toBeVisible({ timeout: 10_000 })
    ...
    // O guard do router redireciona para /login. Esperamos a URL em vez de
    // 'networkidle' porque a navegacao e client-side e nao gera trafego de rede.
